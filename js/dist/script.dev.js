...
$(document).ready(function () {
  var savedItems = JSON.parse(localStorage.getItem("savedItems")) || []; // Dropdown menu functionality
  ...
  function updateSavedItems() {
    ...
    if (savedItems.length === 0) {
      ...
      savedItems.forEach(function (item) {
        savedItemsList.append("<li>".concat(item, "</li>"));
      });
    }
  }
  ...
  $(".save-btn").click(function () {
    ...
    if (!savedItems.includes(item)) {
      ...
      alert("You have ".concat(savedItems.length, " items in your \"Save for Later\" folder."));
      updateSavedItems();
    } else {
  ...
  $("#comment-form").submit(function (event) {
    ...
    alert("Comment submitted: ".concat(comment));
  }); // Like button functionality

  $(".like-btn").click(function () {
    var item = $(this).data("item");
    alert("You liked: ".concat(item));
